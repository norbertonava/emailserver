    ...
    public class SMTP
    {
        public static void SendBadResponse(string toEmailAddress)
        {
            ...
            try
            {
                ...
                Database.SaveLogEntry(LogAction.BAD_PHONE_NUMBER, string.Format("From:{0}", toEmailAddress));
            }
            catch (Exception e)
            {
                Database.SaveLogEntry(LogAction.ERROR, string.Format("{0}:{1}", e.Message, e.StackTrace));
            }
        }
    }
}