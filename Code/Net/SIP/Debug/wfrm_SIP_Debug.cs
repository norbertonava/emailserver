...
namespace Merculia.Net.SIP.Debug
{
    /// <summary>
    /// SIP debug UI.
    /// </summary>
    public class wfrm_SIP_Debug : Form
    {
        ...
        public wfrm_SIP_Debug(SIP_Stack stack)
        {
            ...
            m_pStack = stack;
            m_pStack.Logger.WriteLog += new EventHandler<Merculia.Net.Log.WriteLogEventArgs>(Logger_WriteLog);
        ...
        private void Logger_WriteLog(object sender,Merculia.Net.Log.WriteLogEventArgs e)
        {
            if(!this.Visible){
                return;
            }
