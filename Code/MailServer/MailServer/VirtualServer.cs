...
using Merculia.Net.POP3.Server;
using Merculia.Net.IMAP;
using Merculia.Net.IMAP.Server;
using Merculia.MailServer.Relay;
using Merculia.Net.SIP;
using Merculia.Net.SIP.Stack;
using Merculia.Net.SIP.Proxy;
...
namespace Merculia.MailServer
{
    ...
    public class VirtualServer
    {
        ...
        private void SMTP_Server_SessionLog(object sender,Merculia.Net.Log.WriteLogEventArgs e)
        {
        ...
        private void m_pRelayServer_WriteLog(object sender,Merculia.Net.Log.WriteLogEventArgs e)
        {
        ...
        private void POP3_Server_SessionLog(object sender,Merculia.Net.Log.WriteLogEventArgs e)
        {
        ...
        private void IMAP_Server_SessionLog(object sender,Merculia.Net.Log.WriteLogEventArgs e)
        {
		...
		private void OnServer_SysError(object sender,Merculia.Net.Error_EventArgs e)
		{
			OnError(e.Exception);
		}
        ...
        public void Start()
        {
            ...
            m_pSmtpServer.Error += new Merculia.Net.ErrorEventHandler(OnServer_SysError);
            ...
            m_pPop3Server.Error += new Merculia.Net.ErrorEventHandler(OnServer_SysError);            
            ...
            m_pImapServer.Error += new Merculia.Net.ErrorEventHandler(OnServer_SysError);
        ...
        private void LoadSettings()
		{
			try{
				lock(this){
                    ...
                    Merculia.Net.DNS.Client.Dns_Client.DnsServers = dnsServers.ToArray();
                    ...
                    try{
                        ...
                        if(ConvertEx.ToBoolean(dr["LogRelayCmds"])){
                            m_pRelayServer.Logger = new Merculia.Net.Log.Logger();
                            m_pRelayServer.Logger.WriteLog += new EventHandler<Merculia.Net.Log.WriteLogEventArgs>(m_pRelayServer_WriteLog);
                        }
                        else{
                    ...
                    try{
                        if(ConvertEx.ToBoolean(dr["LogSMTPCmds"],false)){                            
                            m_pSmtpServer.Logger = new Merculia.Net.Log.Logger();
                            m_pSmtpServer.Logger.WriteLog += new EventHandler<Merculia.Net.Log.WriteLogEventArgs>(SMTP_Server_SessionLog);
                        }
                        else{
                            m_pSmtpServer.Logger = null;
                        }
                        if(ConvertEx.ToBoolean(dr["LogPOP3Cmds"],false)){                            
                            m_pPop3Server.Logger = new Merculia.Net.Log.Logger();
                            m_pPop3Server.Logger.WriteLog += new EventHandler<Merculia.Net.Log.WriteLogEventArgs>(POP3_Server_SessionLog);
                        }
                        else{
                            m_pPop3Server.Logger = null;
                        }
                        if(ConvertEx.ToBoolean(dr["LogIMAPCmds"],false)){                            
                            m_pImapServer.Logger = new Merculia.Net.Log.Logger();
                            m_pImapServer.Logger.WriteLog += new EventHandler<Merculia.Net.Log.WriteLogEventArgs>(IMAP_Server_SessionLog);
                        }
                        else{
