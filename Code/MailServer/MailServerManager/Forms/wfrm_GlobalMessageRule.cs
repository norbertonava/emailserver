...
{
    /// <summary>
    /// Global message rule Add/Edit window.
    /// </summary>
    public class wfrm_GlobalMessageRule : Form
    {
        //--- Common UI -------------------------
        ...
        public wfrm_GlobalMessageRule(VirtualServer virtualServer)
        {
        ...
        private void InitUI()
        {
            ...
            mt_Tab_General_Info.Text = "Specify global message rule info.";
        ...
        private void m_pActions_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(m_pTab_Actions_Actions.Items.Count > 0 && m_pTab_Actions_Actions.SelectedItems.Count > 0){
                //m_pDeleteAction.Enabled = true;
                //m_pEditAction.Enabled = true;
            }
            else{
                //m_pDeleteAction.Enabled = false;
                //m_pEditAction.Enabled = false;
            }
        }
        ...
        private void LoadActions()
        {
            ...
            foreach(GlobalMessageRuleActionBase action in m_pRule.Actions){
                ListViewItem it = new ListViewItem();
                //--- Get action human readable text -------//
                if(action.ActionType == GlobalMessageRuleAction_enum.AutoResponse){
                    it.Text = "Auto Response";
                }
                else if(action.ActionType == GlobalMessageRuleAction_enum.AutoResponse){
        ...
        {
            try{
                Merculia.MailServer.GlobalMessageRuleProcessor.CheckMatchExpressionSyntax(m_pTab_General_MatchExpression.Text);
