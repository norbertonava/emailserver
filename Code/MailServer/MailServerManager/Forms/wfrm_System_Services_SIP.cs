    ...
    public class wfrm_System_Services_SIP : Form
    {
        ...
        private void LoadData()
        {
            try{
                ...
                if((settings.ProxyMode & Merculia.Net.SIP.Proxy.SIP_ProxyMode.B2BUA) != 0){
        ...
        private void SaveData(bool confirmSave)
        {
            try{
                ...
                if(m_pTabGeneral_ProxyType.SelectedIndex == 0){
                    settings.ProxyMode = Merculia.Net.SIP.Proxy.SIP_ProxyMode.Registrar | Merculia.Net.SIP.Proxy.SIP_ProxyMode.B2BUA;
                }
                else{
                    settings.ProxyMode = Merculia.Net.SIP.Proxy.SIP_ProxyMode.Registrar | Merculia.Net.SIP.Proxy.SIP_ProxyMode.Statefull;
                }
