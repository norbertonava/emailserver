...
namespace Merculia.MailServer.UI
{
    ...
    public class wfrm_System_General : Form
    {
        ...
        private void m_pTestDns_Click(object sender, EventArgs e)
        {
			...
			DnsServerResponse response = dns.Query("Merculia.ee",QTYPE.MX);
			if(!response.ConnectionOk || response.ResponseCode != RCODE.NO_ERROR){
				MessageBox.Show(this,"Invalid dns server(" + m_pDns1.Text + "), can't resolve Merculia.ee","Info",MessageBoxButtons.OK,MessageBoxIcon.Warning);
				return;
			}
			...
			response = dns2.Query("Merculia.ee",QTYPE.MX);
			if(!response.ConnectionOk || response.ResponseCode != RCODE.NO_ERROR){
				MessageBox.Show(this,"Invalid dns server(" + m_pDns2.Text + "), can't resolve Merculia.ee","Info",MessageBoxButtons.OK,MessageBoxIcon.Warning);
				return;
			}
